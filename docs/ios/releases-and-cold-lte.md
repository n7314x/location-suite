...
Implementation snapshot: 2026-09-13, combining the Phase 1 self-maintenance
addendum with coordinate-free LTE session preparation. This document covers
unsigned release delivery, the legacy SideStore handoff, direct profile-renewal
guidance, and cold phone-local RemotePairing observability/recovery. It does not
change route or saved-location features.
...
when a second device read reports a later expiry. Apple Account/developer-session
Gate 1 is physically confirmed: the app reported `Apple developer session is
ready.` Profile-renewal Gate 2 remains a separate physical-test candidate;
[the Phase 1 document](self-maintenance-signing.md) is authoritative for its
remaining gates. SideStore remains only the older release-install handoff, not
a dependency of the new profile refresh.
